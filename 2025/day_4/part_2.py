...
def remove_paper_rolls(grid: list[list[str]], locations: list[tuple[int, int]]) -> None:
...
while removed_paper_rolls != 0:
    available_rolls_locations = []

    for y in range(0, len(roll_grid)):
        for x in range(0, len(roll_grid[0])):
            if roll_grid[y][x] == "@" and less_than_4_roll_around(x, y, roll_grid):
                available_rolls_locations.append((x, y))
    remove_paper_rolls(roll_grid, available_rolls_locations)
    removed_paper_rolls = len(available_rolls_locations)
    total_paper_rolls_removed += removed_paper_rolls


print(total_paper_rolls_removed)
